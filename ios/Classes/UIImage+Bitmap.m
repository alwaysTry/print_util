...
+ (UIImage *)screenshotWithOptimization:(UIImageView *)imageView
{
...
+ (UIImage *)qrCodeImageWithInfo:(NSString *)info centerImage:(UIImage *)centerImage  width:(CGFloat)width
{
    ...
    UIImageView* newImageView = [[UIImageView alloc] initWithImage:codeImage];
    newImageView.contentMode = UIViewContentModeCenter;
    newImageView.backgroundColor = [UIColor whiteColor];
    newImageView.frame = CGRectMake(0, 0, codeImage.size.width + 30, codeImage.size.height + 30);
    UIImage* bbbbb = [self screenshotWithOptimization:newImageView];
    
    //二维码rect
    CGRect rect = CGRectMake(0, 0, bbbbb.size.width, bbbbb.size.height);
